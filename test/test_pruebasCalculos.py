...
class testcalcularLiquidacion(unittest.TestCase):
    
    def test_fecha_normal(self):
        fecha_inicio_str = "01/01/2006"
        fecha_fin_str = "31/01/2006"

        diferencia = logicaLiquidacion.calcular_tiempo_trabajado_dias(fecha_inicio_str, fecha_fin_str)
    ...
    def test_caso_normal1(self):
        ...
        cesantias = logicaLiquidacion.calcular_cesantias(salario, diferencia)
        interescCesantias = logicaLiquidacion.calcular_interes_cesantias(salario, diferencia)
        vacaciones = logicaLiquidacion.calcular_vacaciones(salario, diferencia)
        primaServicios = logicaLiquidacion.calcular_prima_servicios(salario, diferencia)
        pagoNeto = logicaLiquidacion. calcular_pago_neto(cesantias, interescCesantias, vacaciones, primaServicios)
    ...
    def test_caso_normal2(self):
        ...
        cesantias = logicaLiquidacion.calcular_cesantias(salario, diferencia)
        interescCesantias = logicaLiquidacion.calcular_interes_cesantias(cesantias, diferencia)
        vacaciones = logicaLiquidacion.calcular_vacaciones(salario, diferencia)
        primaServicios = logicaLiquidacion.calcular_prima_servicios(salario, diferencia)
        pagoNeto = logicaLiquidacion. calcular_pago_neto(cesantias, interescCesantias, vacaciones, primaServicios)
    ...
    def test_salario_menorigual_cero(self):
        with self.assertRaises(logicaLiquidacion.errores.ErrorSalarioMenorDeCero):
            logicaLiquidacion.validar_salario(0)
        
    def test_fecha_retiro_anterior_a_fecha_ingreso(self):
        with self.assertRaises(logicaLiquidacion.errores.ErrorFechaIncorrecta):
            logicaLiquidacion.calcular_tiempo_trabajado_dias("10/04/2026", "28/03/2025")

    def test_fecha_ingresada_inexistente(self):
        with self.assertRaises(logicaLiquidacion.errores.ErrorFechaFormatoIncorrecto):
            logicaLiquidacion.calcular_tiempo_trabajado_dias("32/04/2026", "01/05/2026")

    def test_fecha_formato_invalida(self):
        with self.assertRaises(logicaLiquidacion.errores.ErrorFechaFormatoIncorrecto):
            logicaLiquidacion.calcular_tiempo_trabajado_dias("cualquiera", "05/14/2026")
    
    def test_valor_ingresado_en_formato_invalivo(self):
        with self.assertRaises(logicaLiquidacion.errores.ErrorSalarioNoEntero):
